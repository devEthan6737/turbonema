...
export default class FarmCommand extends Command {
    async run(ctx: CommandContext) {
        ...
        if (profile.farm.restart < Date.now()) {
            ...
            profile.farm.used = 0;
        } else if (profile.farm.used >= profile.farm.available) return ctx.write({ content: `¡Te has quedado sin farmeos disponibles! **Se reinician ${Formatter.timestamp(new Date(profile.farm.restart), TimestampStyle.RelativeTime)}**` });
        ...
        collector.run('claim', async (interaction: CollectorInteraction) => {
            ...
            profile.brainrots = addBrainrot(brainrot, profile.brainrots);
            await profiles.set(ctx.author.id, profile);
